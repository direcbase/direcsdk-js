  ...
  async onEvent(path, params, cb) {
    ...
    if (params) {
      ...
      Object.entries(params).forEach(([key, value]) => {
        let valueStr = "";
        if (typeof value === "object") valueStr = JSON.stringify(value);else valueStr = value;
...
class Direcbase {
  constructor(runner, context) {
    if (!runner) this.runner = new QueryRunner();else this.runner = runner;
...
class Direcauth extends Direcbase {
  ...
  async login(body, headers) {
    const {
      tokens
    } = await this.runner.run('auth/login', body, headers);
    if (tokens) {
      if (!isServer()) {
...
exports.Direcadmin = Direcadmin;
exports.Direcauth = Direcauth;
exports.Direccall = Direccall;
exports.Direcstore = Direcstore;
